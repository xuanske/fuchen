import type { FileFlag, FileKind, RawFile } from "./types";

const PHOTO = new Set([
  "jpg", "jpeg", "png", "gif", "webp", "heic", "heif", "bmp", "raw", "dng", "tif", "tiff",
]);
const VIDEO = new Set(["mp4", "mov", "mkv", "avi", "wmv", "flv", "webm", "m4v", "ts"]);
const AUDIO = new Set(["mp3", "wav", "flac", "aac", "m4a", "ogg", "wma"]);
const DOC = new Set([
  "pdf", "doc", "docx", "xls", "xlsx", "ppt", "pptx", "txt", "md", "csv", "rtf", "pages", "numbers", "key",
]);
...
const CACHE_EXT = new Set([
  "tmp", "temp", "log", "old", "bak", "crdownload", "part", "download", "cache", "xltd",
]);
...
const CACHE_PATH = /(^|\/)(temp|tmp|cache|caches|logs|crashdumps|缓存|临时文件|code cache|gpu cache|shadercache|gpucache)(\/|$)/i;
const LEFTOVER_PATH =
  /(discord|slack|teams|code cache|gpu cache|shadercache|npm-cache|\.npm|pip\/cache|vscode\/logs|obs-studio\/logs|adobe.*media cache)/i;
...
export function classifyFlags(
  ...
  const inDownloads = /下载|downloads|desktop|桌面/i.test(path);
  ...
  if (inDownloads && ageDays >= 365) flags.push("stale");
